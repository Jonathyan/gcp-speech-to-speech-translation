...
async def process_pipeline(audio_chunk: bytes) -> Optional[bytes]:
    ...
    # PHASE 1 + PHASE 2: Use buffered STT WITH the buffer state fix
    # The smart_buffer.py clear() method now preserves first_chunk_time (Phase 1 fix)
    # Combined with Phase 2 WAV audio processing
    text_result = await buffered_speech_to_text(audio_chunk)
    
    if text_result is None:
        # Still buffering - no output yet (this is normal behavior)
        return None
